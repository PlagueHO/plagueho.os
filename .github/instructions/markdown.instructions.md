...
All markdown content in this repository **must** follow the
...
1. **Tables**: Use GFM pipe tables for tabular data. Ensure proper formatting, alignment, and consistent column counts.
...
- **Tables**: Use GFM pipe tables (`|`) with a header row and delimiter row. Ensure that columns are properly aligned, use leading and trailing pipes, and maintain a consistent column count per row.
- **Task Lists**: Use GFM task list syntax (`- [ ]` / `- [x]`) for checklists.
- **Strikethrough**: Use GFM double-tilde syntax (`~~text~~`) for strikethrough text.
